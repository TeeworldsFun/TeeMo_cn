...
#include "gamemodes/teemo.h" // TeeMo
#include <game/server/entities/laserTurret.h> // TeeMo
...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage)
{
	...
	if (!NoDamage)
	{
		...
		// TeeMo
        Num = m_World.FindEntities(Pos, Radius, (CEntity**)apEnts, MAX_CLIENTS, CGameWorld::ENTTYPE_LASER);
        for(int i = 0; i < Num; i++)
        {
            CLaserTurret *pLaserTurret = dynamic_cast<CLaserTurret*>(apEnts[i]);
            if (pLaserTurret)
            {
                vec2 Diff = apEnts[i]->m_Pos - Pos;
                float l = length(Diff);
                l = 1-clamp((l-InnerRadius)/(Radius-InnerRadius), 0.0f, 1.0f);
                float Dmg = 6 * l;
                pLaserTurret->TakeDamage((int)Dmg, Owner);
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	m_pController->OnClientEnter(ClientID); // TeeMo
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		if(MsgID == NETMSGTYPE_CL_SAY)
		{
            ...
            if (m_pController->OnChat(ClientID, Team, pMsg->m_pMessage))
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("vote", "r", CFGFLAG_SERVER, ConVote, this, "Force a vote to yes/no");
		...
		// TeeMo
	Console()->Register("give_weapon", "ii", CFGFLAG_SERVER, ConGiveWeapon, this, "Give weapon to player");
	Console()->Register("give_bonus", "ii", CFGFLAG_SERVER, ConGiveBonus, this, "Give bonus to player");
	Console()->Register("bonus_list", "", CFGFLAG_SERVER, ConBonusList, this, "View bonus id list");
	Console()->Register("weapons_list", "", CFGFLAG_SERVER, ConWeaponsList, this, "View weapons id list");
...
void CGameContext::OnInit(/*class IKernel *pKernel*/)
{
    ...
    else if(str_comp_nocase(g_Config.m_SvGametype, "teemo") == 0)
        m_pController = new CGameControllerTEEMO(this);
	else
...
void CGameContext::ConGiveWeapon(IConsole::IResult *pResult, void *pUserData)
{
    ...
    if (pSelf->m_apPlayers[ClientID] && pSelf->m_apPlayers[ClientID]->GetCharacter())
    {
        ...
        pSelf->m_apPlayers[ClientID]->GetCharacter()->GiveWeapon(Weapon, 10);
        pSelf->SendChatTarget(ClientID, buff);
    }
    else
        pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "Invalid client id to give weapon");
}
void CGameContext::ConGiveBonus(IConsole::IResult *pResult, void *pUserData)
{
    CGameContext *pSelf = (CGameContext *)pUserData;
    int ClientID = pResult->GetInteger(0);
    int Bonus = pResult->GetInteger(1);
    ...
    if (pSelf->m_apPlayers[ClientID])
    {
        ...
        pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "Invalid client id to give bonus");
}
void CGameContext::ConBonusList(IConsole::IResult *pResult, void *pUserData)
{
    CGameContext *pSelf = (CGameContext *)pUserData;
    const char *bonusNames[4] = { "FLY", "GOD", "HOOKER", "INVISIBLE" };
    char buff[128];
    ...
    for (int i=0; i<4; i++)
    {
        str_format(buff, sizeof(buff), "%d. %s", i, bonusNames[i]);
        pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", buff);
    }
}
void CGameContext::ConWeaponsList(IConsole::IResult *pResult, void *pUserData)
{
    CGameContext *pSelf = (CGameContext *)pUserData;
    const char *bonusNames[15] = { "HAMMER", "GUN", "SHOTGUN", "GRENADE", "RIFLE", "NINJA", "PANTHOM GRENADE", "AIR GRENADE", "LASER GRENADE", "DRAGON GRENADE", "EXTRA BOMB", "PLASMA", "LASER HEALER", "LASER REPAIR", "TURRET" };
    char buff[128];

    pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", " ");
    pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "WEAPONS LIST");
    pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "========= === =");
    for (int i=0; i<15; i++)
    {
        str_format(buff, sizeof(buff), "%d. %s", i, bonusNames[i]);
