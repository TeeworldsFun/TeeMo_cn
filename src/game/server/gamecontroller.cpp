...
// TeeMo
bool IGameController::OnChat(int ChatterClientID, int Team, const char *pText)
{
    return true;
}
void IGameController::OnClientEnter(int ClientID)
{ }
void IGameController::OnEndRound()
{ }
...
bool IGameController::IsFriendlyFire(int ClientID1, int ClientID2)
{
	...
	if(IsTeamplay())
	{
        ...
        if ((GameServer()->m_apPlayers[ClientID1]->GetInfoTEEMO()->GetRole() == ROLE_SPY || GameServer()->m_apPlayers[ClientID2]->GetInfoTEEMO()->GetRole() == ROLE_SPY) &&
...
bool IGameController::CanJoinTeam(int Team, int NotThisID)
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(GameServer()->m_apPlayers[i] && i != NotThisID)
		{
			if(str_comp_nocase(g_Config.m_SvGametype, "teemo") == 0) // TeeMo
			{
				if (GameServer()->m_apPlayers[i]->GetInfoTEEMO()->GetRealTeam() >= TEAM_RED && GameServer()->m_apPlayers[i]->GetInfoTEEMO()->GetRealTeam() <= TEAM_BLUE)
					aNumplayers[GameServer()->m_apPlayers[i]->GetTeam()]++;
			} else if(GameServer()->m_apPlayers[i]->GetTeam() >= TEAM_RED && GameServer()->m_apPlayers[i]->GetTeam() <= TEAM_BLUE)
				aNumplayers[GameServer()->m_apPlayers[i]->GetTeam()]++;
		}
	}
...
bool IGameController::CheckTeamBalance()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
        ...
        {
            if(str_comp_nocase(g_Config.m_SvGametype, "teemo") == 0 && pP->GetInfoTEEMO()) // TeeMo
                aT[pP->GetInfoTEEMO()->GetRealTeam()]++;
            else
                aT[pP->GetTeam()]++;
        }
	}
